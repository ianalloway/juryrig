...
from juryrig import MockJudge, Panel, position_bias, self_consistency, verbosity_bias
...
def audit(judge, label):
    ...
    print(f"position bias   flip_rate={pos.flip_rate:.0%} first_slot_wins={pos.first_slot_wins:.0%} flagged={pos.flagged}")
    verb = verbosity_bias(judge, PAIRS, RUBRIC)
    print(f"verbosity bias  mean_delta={verb.mean_delta:+.3f} flagged={verb.flagged}")
    cons = self_consistency(judge, prompt=CASES[0][0], response=CASES[0][1], rubric=RUBRIC)
    print(f"consistency     spread={cons.spread:.3f} flagged={cons.flagged}")
    return pos.flagged or verb.flagged or cons.flagged


def main():
    fair = MockJudge(name="fair-judge")
    rigged = MockJudge(name="rigged-judge", position_bias=2.0, verbosity_bias=0.4)

    fair_flagged = audit(fair, "fair judge")
    rigged_flagged = audit(rigged, "rigged judge (always prefers slot A, rewards padding)")
    ...
    print(f"scores={report.scores} pooled={report.pooled:.3f} agreement={report.agreement:.3f}")
